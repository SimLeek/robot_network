...
# grabbed off the real X11 desktop into the virtual camera. -1/-1 (the
# default) means passthrough: no forced size/format at all, not even
# yuy2 -- see DesktopVideoFeeder. This is NOT the transmitted
# resolution: CamMicSpkRobotHardware's GstSender downscales from this
# down to settings['cam_res']/['cam_fps'] (endpoint/settings.py, small
# by default -- most AI consumers can't usefully handle 1080p30) on its
# own, exactly like it would for a real webcam. Raise cam_res in
# ~/.robotar/settings.json if a human viewer wants more transmitted
# detail; don't do it by raising these.
CAPTURE_WIDTH  = -1
CAPTURE_HEIGHT = -1
CAPTURE_FPS    = 15
